...
public class CheckInContoller {
	...
	/* @HystrixCommand(fallbackMethod = "UpdateBookingFallback") */
	 	public String updateBooking (Booking booking, 
	 			                                           @PathVariable("booking_id") long pnr,
	 			                                           @PathVariable("mailId") String mailId) throws InterruptedException {
		booking.setChecked_in(true);
	restTemplate.put("https://FlightBooking/FlightBooking/booking/"+pnr+"/"+mailId, booking);
	...
	Booking booking=restTemplate.getForObject("https://FlightBooking/FlightBooking/BookedFlight/"+booking_id, Booking.class);
	Thread.sleep(3000);
		return booking ;
